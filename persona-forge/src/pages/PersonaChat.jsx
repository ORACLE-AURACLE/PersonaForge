...
export default function PersonaChat() {
  ...
  const handleSend = () => {
    ...
    const mockResponse = `Hello! This is a mock response from ${firstName}. You said: "${message}"`;
  ...
  return (
    <section className="chat-page">
      ...
      <div className="chat-body">
        {/* CHAT */}
        <div
          className={`chat-panel ${activeTab !== "chat" ? "hide-mobile" : ""}`}
        >
          {messages.length === 0 ? (
            <div className="chat-empty">
              <p>Start a conversation with {firstName}</p>
              <span>
                Ask about their needs, pitch your idea, or explore how they make
                decisions.
              </span>
            </div>
          ) : (
        ...
        {/* INSIGHTS */}
        <aside
          className={`insights-panel ${activeTab !== "insights" ? "hide-mobile" : ""}`}
        >
          <h4>Insights</h4>
          {insights.length === 0 ? (
